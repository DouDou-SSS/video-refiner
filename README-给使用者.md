...
新建任务会在“抽帧与文案”后自动生成证据时间线：软件先本地计算场景变化和代表帧，再把关键画面与对应文案交给模型做一次视觉标注。5 个单视频维度与 Benchmark 汇总复用这份证据，不重复独立读同一批帧。

在“任务”页面，每个已完成视频卡片右下角有查看按钮。点击后可查看关键帧、对应时间段、关联文案、屏幕文字和 evidence ID。没有可靠文案、证据时间线损坏或模型输出不合格时，任务会显示失败/等待重试，不会用占位结果完成或导出。
