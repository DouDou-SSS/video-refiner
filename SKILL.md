...
- **视频证据时间线** — 证据窗口与检测切段分开记录，包含关键帧、图文时间关联和一次性视觉标注；只有时间对齐和检测切段均可靠时才生成精确时序结论
...
- **v8.3 (2026-06-20)** — 新增证据时间线与 evidence ID：视觉判断可回溯到具体时间段，导出包携带轻量时间线 JSON
...
FunASR 本地模型 → 自动标点 + 分段
    ↓
证据时间线：场景变化 + 覆盖关键帧 + 图文时间关联 → 一次性视觉标注与质量门禁
    ↓
...
导出给 VideoAutomation 前必须通过质量门：五维分析、Card、Notes 和 Index 数量及 ID 一致；核心字段非空；无模型拒答、模板占位和本机绝对路径。新版任务还要求视觉时间线完整、Card 引用稳定镜头 evidence ID、视觉覆盖状态为 complete。导出只携带轻量时间线 JSON，不携带帧图、视频或完整转写。失败时生成 `_FAILED_` 目录中的 `validation_report.json`，不得显示导出成功。
...
- Benchmark Intelligence 结构化产物禁止保守降级生成；模型拒答、坏 JSON、缺字段或超时必须提示用户并按间隔自动重试，直到模型返回通过校验的结果或用户取消任务。
